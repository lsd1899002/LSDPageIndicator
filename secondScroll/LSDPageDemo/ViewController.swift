...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let arr = ["精选","秒杀","电器","时尚","生活","超市"]
        var beanArr: [MyInputBean] = []
        var i = 0
        arr.forEach { (str) in
            let bean = MyInputBean()
            bean.title = str
            i += 1
            beanArr.append(bean)
        }
        let item = MyInput(datas: ["item1","item2","item3"])
        indicator.show(inputView: MyInput(datas: beanArr), slider: mySlider(), selectedIndex: 3)
